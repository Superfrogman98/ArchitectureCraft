...
public class ShapeTE extends BaseTileEntity {
	...
	public void toggleConnectionGlobal(EnumFacing dir) {
		...
		if (nte != null)
			nte.setConnectionEnabledGlobal(oppositeFacing(dir), newState);
	}
	...
	public boolean canRenderInLayer(EnumWorldBlockLayer layer) {
		if (blockCanRenderInLayer(baseBlockState.getBlock(), layer))
			return true;
		if (secondaryBlockState != null)
			return blockCanRenderInLayer(secondaryBlockState.getBlock(), layer);
		return false;
	}
	...
	public ShapeTE getConnectedNeighbourGlobal(EnumFacing dir) {
		if (worldObj != null) {
			if (connectionIsEnabledGlobal(dir)) {
				ShapeTE nte = getNeighbourGlobal(dir);
				if (nte != null && nte.connectionIsEnabledGlobal(oppositeFacing(dir)))
					return nte;
			}
		}
		return null;
