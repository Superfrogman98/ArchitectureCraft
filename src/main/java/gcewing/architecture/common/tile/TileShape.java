...
public class TileShape extends TileArchitecture {
    ...
    public static TileShape get(IBlockAccess world, BlockPos pos) {
        if (world != null) {
            ...
            if (te instanceof TileShape) return (TileShape) te;
        }
        return null;
    }
    ...
    public TileShape() {
        ...
        baseBlockState = getDefaultBlockState(Blocks.planks);
    }
    ...
    public TileShape(Shape s, Block b, int d) {
        super();
        shape = s;
        baseBlockState = getBlockStateFromMeta(b, d);
    }
    ...
    public void setConnectionEnabledGlobal(EnumFacing dir, boolean state) {
        ...
        if (state) disabledConnections &= ~bit;
        else disabledConnections |= bit;
        markChanged();
    }
    ...
    public void toggleConnectionGlobal(EnumFacing dir) {
        ...
        if (nte != null) nte.setConnectionEnabledGlobal(oppositeFacing(dir), newState);
    }
    ...
    protected void readShapeFromNBT(NBTTagCompound nbt) {
        ...
        if (baseBlockState == null) baseBlockState = getDefaultBlockState(Blocks.planks);
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (offsetX != 0) nbt.setByte("offsetX", offsetX);
    }
    ...
    protected void writeShapeToNBT(NBTTagCompound nbt) {
        ...
        if (disabledConnections != 0) nbt.setInteger("Disconnected", disabledConnections);
    }
    ...
    protected void nbtSetBlockState(NBTTagCompound nbt, String nameField, String dataField, IBlockState state) {
        if (state != null) {
            ...
            nbt.setInteger(dataField, getMetaFromBlockState(state));
        }
    }
    ...
    public boolean applySecondaryMaterial(ItemStack stack, EntityPlayer player) {
        ...
        if (item instanceof ItemCladding && shape.kind.acceptsCladding())
            ...
            if (block != null) {
                IBlockState state = getBlockStateFromMeta(block, getStackMetadata(stack));
        ...
        if (materialState != null) {
            if (secondaryBlockState == null) {
                ...
                if (!Utils.playerIsInCreativeMode(player)) --stack.stackSize;
            }
            return true;
        } else return false;
    }
    ...
    public boolean canRenderInLayer(EnumWorldBlockLayer layer) {
        if (blockCanRenderInLayer(baseBlockState.getBlock(), layer)) return true;
        if (secondaryBlockState != null) return blockCanRenderInLayer(secondaryBlockState.getBlock(), layer);
        return false;
    }

    public TileShape getNeighbourGlobal(EnumFacing dir) {
        return TileShape.get(worldObj, getPos().offset(dir));
    }
    ...
    public TileShape getConnectedNeighbourGlobal(EnumFacing dir) {
        if (worldObj != null) {
            if (connectionIsEnabledGlobal(dir)) {
                TileShape nte = getNeighbourGlobal(dir);
                if (nte != null && nte.connectionIsEnabledGlobal(oppositeFacing(dir))) return nte;
            }
        }
        return null;
    }
